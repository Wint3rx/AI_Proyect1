...
public class BotCuestionario extends TelegramLongPollingBot {
    ...
    public void botCuestionario() {
        ...
        preguntas.put("SECTION_4", new String[]{"4.1- Hagamos un repaso de ti: Que te gusta?","4.2- Has leido un libro? ","4.3- Te gusta la musica?"});
    }
    ...
    private void manejaCuestionario(long chatId, String response) {
        ...
        if (indicePregunta.get(chatId) == 1 ) {
            ...
            if (intresponse<5){
                ...
                sendText(chatId, "No eres apto para continuar\nPorfavor coloca otra edad");
                enviarPregunta(chatId);
            } else if (intresponse>95) {
                sendText(chatId, "Tu respuesta fue: " + response);
                sendText(chatId,"Seguro que esta es tu edad?\nPorfavor colocar otra edad");
                enviarPregunta(chatId);
            }else {
